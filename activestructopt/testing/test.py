...
def main():
	...
	pristine_structure = CifParser("datasets/ht/start/" + str(sys.argv[1]) + ".cif"
		).get_structures(primitive = False)[0]
	target_structure = CifParser("datasets/ht/target/" + str(sys.argv[1]) + ".cif"
	...
	al.save("res/" + str(sys.argv[1]) + ".pkl", 
