...
def mcmc_step(structure, tol):
    for i in range(len(structure)):
        structure.sites[i].a = (structure.sites[i].a + np.random.uniform(0., tol) ) % 1
        structure.sites[i].b = (structure.sites[i].b + np.random.uniform(0., tol) ) % 1
        structure.sites[i].c = (structure.sites[i].c + np.random.uniform(0., tol) ) % 1
    return structure
